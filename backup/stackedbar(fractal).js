    ...
    var svg = d3.select("#stackedbarVis").append("svg")
     ...
     // debugger;
      ...
      data.forEach(function(d) {
        var y0 = 0;
        var lastvalue = d.values.length -1;
        d.storages = color.domain().map(function(name) { return {name: name, y0: y0, y1: y0 += +d.values[lastvalue].storage}; });
        d.total = d.storages[d.storages.length - 1].y1;
      });
